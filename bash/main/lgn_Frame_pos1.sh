...
then
        ...
        for t_2 in $t_2 1.00
        do 
        name1="ICDE_rebuttal_${dataset}_${loss_name}_${gnn_name}_t1_${t_1}_t2_${t_2}_t3_${t_3}_HOPS_${context_hops}_${bsz}_${n_negs}_lr_${lr}_l2_${l2}_NODROP_${loss_re}_MODE_${pos_mode}_P_${t_patience}_${sampling_method}_G_${generate_mode}_DIM_${dim}"
        echo $name1
        CUDA_VISIBLE_DEVICES=$gpus python main.py --name $name1 --dataset $dataset --gnn $gnn --dim 64 --lr $lr \
                                --batch_size $bsz --gpu_id 0 --logdir $logdir \
                                --n_negs  $n_negs --l2 $l2\
