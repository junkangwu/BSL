...
class mf_frame_align(nn.Module):
    ...
    def Uniform_loss(self, user_gcn_emb, pos_gcn_emb, neg_gcn_emb, user, step):
        ...
        y_pred = torch.bmm(item_e, u_e.unsqueeze(-1)).squeeze(-1) # [B M+1]
        # uniform
        uniform_score_item = torch.pdist(item_e[:, 0], p=2).pow(2).mul(-2).exp().mean().log()
        uniform_score_user = torch.pdist(u_e, p=2).pow(2).mul(-2).exp().mean().log()
        uniform_score = uniform_score_item * 0.5 + uniform_score_user * 0.5
        aligh_score = (u_e - item_e[:, 0]).norm(dim=1).pow(2).mean()
        ...
        return loss + emb_loss, emb_loss, (aligh_score, uniform_score)
