...
class MF_pos2(nn.Module):
    ...
    def forward(self, batch=None):
        ...
        return self.Uniform_loss(self.user_embed[user], self.item_embed[pos_item], self.item_embed[neg_item], self.item_embed[pos_item_], user)
    ...
    # 对比训练loss，仅仅计算角度
    def Uniform_loss(self, user_gcn_emb, pos_gcn_emb, neg_gcn_emb, pos_gcn_emb_, user):
        ...
        pos_e_ = pos_gcn_emb_
        # print(pos_e_)
        item_e = torch.cat([pos_e.unsqueeze(1), pos_e_, neg_e], dim=1) # [B, M+1, F]
        # print(item_e.size())
        if self.u_norm:
