...
class MF_output_score(nn.Module):
    def __init__(self, data_config, args_config, adj_mat, logger=None):
        super(MF_output_score, self).__init__()
    ...
    def Uniform_loss(self, user_gcn_emb, pos_gcn_emb, neg_gcn_emb, user):
        ...
        # output and logout
        pos = y_pred[:, 0].view(-1, 1)
        neg = y_pred[:, 1:]
        score_neg_mean = torch.mean(neg, dim=1, keepdim=True).half().cpu().detach().numpy()
        score_pos_mean = pos.half().cpu().detach().numpy()
        score_pos_mean_ = (pos / self.temperature).half().cpu().detach().numpy()
        scores = np.concatenate((score_neg_mean, score_pos_mean, score_pos_mean_), axis=1)
        ...
        return loss.mean() + emb_loss, emb_loss, scores
