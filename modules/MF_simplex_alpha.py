...
class MF(nn.Module):
    def __init__(self, data_config, args_config, adj_mat, logger=None):
        ...
        elif self.loss_name == "RinceLoss":
            self.loss_fn = losses.RinceLoss(self.temperature, self.temperature_2, self.temperature_3)
        elif self.loss_name == "DRO_reweightLoss":
            self.loss_fn = losses.DRO_reweightLoss(self.temperature, self.temperature_2, self.temperature_3)
    ...
    def forward(self, batch=None):
        ...
        if self.sampling_method == "uniform_once":
            return self.Uniform_loss_Once(self.user_embed[user], self.item_embed[pos_item], self.item_embed[neg_item], user)
        else:
    ...
    def update_alpha(self, batch):
        user_gcn_emb = self.user_embed[batch['users']]
        neg_gcn_emb = self.item_embed[batch['neg_items']]
        u_e = user_gcn_emb  # [B, F]
        if self.mess_dropout:
            u_e = self.dropout(u_e)
        neg_e = neg_gcn_emb # 

        h_x = torch.bmm(neg_e, u_e.unsqueeze(-1)).squeeze(-1).detach()
        v_alpha = self.temperature * torch.log( torch.mean( torch.exp(h_x / self.temperature))) + self.temperature * self.eta
        return v_alpha
        
    def update_alpha_newton(self, batch):
        user_gcn_emb = self.user_embed[batch['users']]
        neg_gcn_emb = self.item_embed[batch['neg_items']]
        u_e = user_gcn_emb  # [B, F]
        if self.mess_dropout and self.mix_replace:
            u_e = self.dropout(u_e)
        neg_e = neg_gcn_emb # 
        if self.u_norm:
        ...
        # h_x = torch.bmm(neg_e, u_e.unsqueeze(-1)).squeeze(-1).view(-1).detach()
        h_x = torch.bmm(neg_e, u_e.unsqueeze(-1)).squeeze(-1).detach()
        cnt = 0
        epsilon = 999
        while epsilon > self.temperature_3:
            cnt +=1
            if cnt > self.cnt_lr:
                break
            diff1 = torch.log( torch.mean(torch.exp(h_x / self.temperature.unsqueeze(1)), dim=1)) - \
                1. / self.temperature * torch.sum( torch.exp(h_x / self.temperature.unsqueeze(1)) * h_x, dim=1) / torch.sum(torch.exp(h_x / self.temperature.unsqueeze(1)), dim=1) + \
                    self.eta
            diff2 = 1. / torch.pow(self.temperature, 3) * torch.var(h_x, dim=1)
            self.temperature = torch.clamp(self.temperature - diff1 / diff2, min=self.tau_min)
            # epsilon = self.tem
    ...
    def Uniform_loss(self, user_gcn_emb, pos_gcn_emb, neg_gcn_emb, user):
        ...
        elif self.loss_name == "RinceLoss" or self.loss_name == "DCL_Loss" or self.loss_name == "DRO_reweightLoss":
    ...
    def Uniform_loss_Once(self, user_gcn_emb, pos_gcn_emb, neg_gcn_emb, user):
        batch_size = user_gcn_emb.shape[0]
        u_e = user_gcn_emb  # [B, F]
        if self.mess_dropout:
            u_e = self.dropout(u_e)
        ...
        if self.i_norm:
            ...
            neg_e = F.normalize(neg_e, dim=-1)
        y_pred_pos = torch.mul(u_e, pos_e).sum(dim=-1, keepdim=True)
        y_pred_neg = torch.mm(u_e, neg_e.t())
        # y_pred = torch.bmm(item_e, u_e.unsqueeze(-1)).squeeze(-1) # [B M+1]
        y_pred = torch.cat([y_pred_pos, y_pred_neg], dim=1)
