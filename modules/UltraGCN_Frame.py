...
class Ultra_gcn_vanilla(nn.Module):
    ...
    def cal_loss_L(self, users, pos_items, neg_items, omega_weight):
        ...
        # pos_neg_score
        pos_score = pos_scores.view(-1, 1).half().cpu().detach().numpy()
        neg_score = neg_scores.mean(dim=-1, keepdim=True).half().cpu().detach().numpy()
        scores = np.concatenate((pos_score, neg_score), axis=1)
        loss = pos_loss + neg_loss * self.negative_weight
      
        return loss.sum(), scores
    ...
    def forward(self, batch=None, cluster_result=None):
        ...
        if self.loss_name == "bce":
            ...
            loss, scores = self.cal_loss_L(users, pos_items, neg_items, omega_weight)
            ...
            neg_items = batch['neg_items']
            omega_weight = self.get_omegas(users, pos_items, neg_items)
            loss, scores = self.cal_CL_sigmoid_in_loss_L(users, pos_items, neg_items, omega_weight)
            loss += self.gamma * self.norm_loss()
            loss += self.lambda_ * self.cal_loss_I(users, pos_items)
    ...
    def Sample_loss(self, users, pos_items, neg_items, omega_weight):
        ...
        y_pred = torch.bmm(item_e, user_embeds.unsqueeze(-1)).squeeze(-1) # [B M+1]
